...
class Delivery_internal_nondoc extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('download/delivery_internal_nondoc_m');  
    ...
    function home() {
        $menuId = $this->home_m->get_menu_id('download/delivery_internal_nondoc/home');
        ...
        // $data['getExpedition'] = $this->delivery_internal_nondoc_m->getExpedition();
        $this->template->set('title', 'home');
        $this->template->load('default_template/template9', 'download/delivery_internal_nondoc_v', $data);
    }

    public function getDevAll() {
        $rows = $this->delivery_internal_nondoc_m->getDevAll();
        $data['data'] = array();
        $no = 0;
        foreach ($rows as $row) {
            $no++;
    ...
    function downloadExcel(){
        ...
        $excel->getProperties()->setCreator('Mailroom AG')
                 ...
                 ->setTitle("Data Delivery internal non doc")
        ...
        $excel->setActiveSheetIndex(0)->setCellValue('A1', "DATA DELIVERY INTERNAL NON DOC"); // Set kolom A1 dengan tulisan "DATA SISWA"
        ...
        for($k=0; $k<$countRegNum; $k++){
            ...
            $model = $this->delivery_internal_nondoc_m->update_doc_reg($data,$idDocReg);
            $rows  = $this->delivery_internal_nondoc_m->getDataReg($idDocReg);
            ...
            foreach($rows as $row)
            {
                ...
                $excel->setActiveSheetIndex(0)->setCellValue('G'.$numrow, $row->nrecipient);
        ...
        $excel->getActiveSheet(0)->setTitle("DATA NON DOC");
        ...
        ob_end_clean();
        header( "Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" );
        header('Content-Disposition: attachment; filename="'.$datefile.'_delivery_internal_nondoc.xlsx"');
