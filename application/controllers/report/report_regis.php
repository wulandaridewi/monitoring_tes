...
class Report_regis extends CI_Controller {
    ...
    function getListDoc() {
        ...
        $rows = $this->report_regis_m->getListRegis($startDate,$endDate,$idUser,$usergroup);
        
        $data = array(
            'getListRegis'  => $rows,
        );  

        $this->load->view('report/list_regis_v', $data); 
    }
    ...
    function downloadExcel(){
        ...
        $usergroup   = trim($this->session->userdata('usergroup'));
        $rows        = $this->report_regis_m->getListRegisDownload($startDate,$endDate,$idUser,$usergroup);
        ...
        $excel->setActiveSheetIndex(0)->setCellValue('A1', "Report Registrasi Document"); // Set kolom A1 dengan tulisan "DATA SISWA"
        ...
        $numrow = 4; // Set baris pertama untuk isi tabel adalah baris ke 4        
            ...
            foreach($rows as $row){
                ...
                $excel->setActiveSheetIndex(0)->setCellValue('B'.$numrow, trim($row->register_doc_id));
                $excel->setActiveSheetIndex(0)->setCellValue('C'.$numrow, date('d-m-Y H:i:s',strtotime($row->create_date)));
                $excel->setActiveSheetIndex(0)->setCellValue('D'.$numrow, trim($row->type));
                $excel->setActiveSheetIndex(0)->setCellValue('E'.$numrow, trim($row->doc_description));
        ...
        $excel->getActiveSheet(0)->setTitle("Report Registrasi Document");
        $excel->setActiveSheetIndex(0);
        ob_end_clean();
        header( "Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" );
        header('Content-Disposition: attachment; filename="report_regis_doc.xlsx"');
