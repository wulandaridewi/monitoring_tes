...
class My_container_ extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('container/my_container_m_');
    ...
    function home() {
        $menuId              = $this->home_m->get_menu_id('container/my_container_/home');
        ...
        if ($this->session->userdata('id_user') == '') {
            ...
            //$data['getCategoryDoc'] = $this->my_container_m_->getCategoryDoc();
            $data['getUser']    = $this->global_m->getUser();
            $this->template->set('title', 'home');
            $this->template->load('default_template/template9', 'container/my_container_v_', $data);
        }
    ...
    public function getCollection() {
        ...
        $rows   = $this->my_container_m_->getCollection($idUser,$usergroup);
    ...
    public function getSubFolder() {
        ...
        $usergroup = trim($this->session->userdata('usergroup')); 
        // echo $userid;
        $rows   = $this->my_container_m_->getSubFolder($folder_id,$idUser,$usergroup);
        $data['data'] = array();        
        foreach ($rows as $row) {
            $data = array(
                'folder_id'      => trim($row->folder_id),
    ...
    public function getOpenSubFolder() {
        ...
        $idUser    = trim($this->session->userdata('id_user'));
        $usergroup = trim($this->session->userdata('usergroup'));
        $rows   = $this->my_container_m_->getOpenSubFolder($sub_folder,$idUser,$usergroup);
        $data['data'] = array();
        $no = 0;
        foreach ($rows as $row) {
            $no++;
    ...
    public function getListDoc() {
        ...
        $getFolderID   = $this->my_container_m_->getFolderID($sub_folder,$document_id,$idUser,$usergroup);
        // echo "<pre>";
        //     print_r($getFolderID);
        //     echo "</pre>";
        $idTabel = 0;
        if (empty($getFolderID)) {
            ...
            foreach ($getFolderID as $key => $value) {
                ...
                $getGeneralIndexName  = $this->my_container_m_->getGeneralIndexName($folderId,$sub_folder);
                $getFieldNameIndexSpecific = $this->my_container_m_->getFieldNameIndexSpecific($document_id);
                ...
                // $getTransDoc = $this->my_container_m_->getTransDoc($sub_folder,$document_id);
                $getSpecificIndexNameTable = $this->my_container_m_->getSpecificIndexNameTable($document_id,$sub_folder,$col,$idUser,$usergroup);
    ...
    public function getKetFile() {
        ...
        $getGeneralIndexName  = $this->my_container_m_->getGeneralIndexName($folder_id,$subFolder);
        $getSpecificIndexName = $this->my_container_m_->getSpecificIndexName($document_id,$trans_doc_id,$idUser);
        $data = array(
            'getGeneralIndexName'  => $getGeneralIndexName,
    ...
    public function getCollectionSub() {
        ...
        $idUser       = trim($this->session->userdata('id_user'));
        $usergroup = trim($this->session->userdata('usergroup'));
        $rows = $this->my_container_m_->getCollectionSub($folder_id,$idUser,$usergroup);
        $data['data'] = array();
        $no = 0;
        foreach ($rows as $row) {
            $no++;
    ...
    function getFieldAll() {
        ...
        $rows = $this->my_container_m_->getFieldAll($categoryDoc);
    ...
    function getFieldAllEdit() {
        ...
        $rows = $this->my_container_m_->getFieldAllEdit($trans_doc_id);
    ...
    function getUserApproval() {       
        $trans_doc_id = $_GET['trans_doc_id'];
        $rows = $this->my_container_m_->getUserApproval($trans_doc_id);
    ...
    function getstatusApproval() {       
        $trans_doc_id = $_GET['trans_doc_id'];
        $idUser       = trim($this->session->userdata('id_user'));
        $rows = $this->my_container_m_->getstatusApproval($trans_doc_id,$idUser);
    ...
    function getFieldGeneralAll() {
        ...
        $rows = $this->my_container_m_->getFieldGeneralAll($folder_id);
    ...
    //     $rows = $this->my_container_m_->getFieldGeneralAllAndValue($folder_id,$sub_folder);
    ...
    function getFieldGeneralAllAndValue() {
        ...
        $rows = $this->my_container_m_->getFieldGeneralAllAndValue($folder_id,$subFolder);
    ...
    function simpanFolder(){
        ...
        $model = $this->my_container_m_->insert_folder($nameGeneralArr,$nameGeneralIDArr,$totalNameGeneral,$date,$folderID,$datetime,$idUser,$subFolder,$general_index_format);
        //echo $model;
    ...
    function editFolder(){
        ...
        $model = $this->my_container_m_->edit_folder($idUser,$datetime,$date,$folderIDEditSf,$nameGeneralArr,$nameGeneralArrNew,$totalNameGeneral,$nameGeneralIDArr,$transIndexGeneralIDArr,$general_index_format,$subFolderEditSf,$subFolderEditSfNew,$directory,$directoryNew);
        //echo $model;        
        if($model == 1){
            $array = array(
            'act' => 1,
    ...
    function hapus() {
        ...
        $model = $this->my_container_m_->deletefolder($folder_id,$data);
        if ($model) {
            $array = array(
                'act' => 1,
                'tipePesan' => 'success',
    ...
    function DeleteSubFolder() {
        ...
        $model = $this->my_container_m_->DeleteSubFolder($sub_folder,$data);
        if ($model) {
            $array = array(
                'act' => 1,
                'tipePesan' => 'success',
    ...
    function deleteDocument() {
        ...
        $model = $this->my_container_m_->deleteDocument($trans_doc_id,$data);
        if ($model) {
            $array = array(
                'act' => 1,
                'tipePesan' => 'success',
    ...
    public function simpan(){
        ...
        if (file_exists($directory)) {
           $model = $this->my_container_m_->insert_all($nameGeneralArr,$nameGeneralIDArr,$totalNameGeneral,$date,$folderID,$datetime,$idUser,$subFolder,$nameSpecificArr,$nameSpecificIDArr,$totalNameSpecfic,$categoryDocVal,$directory,$fileNameUpload,$formatSpesificIDArr,$general_index_format,$fileSize);
    ...
    public function editSetApproval(){        
        ...
        $date     = date("Y-m-d");
        $idUser   = $this->session->userdata('id_user');
        //$model = $this->my_container_m_->insertSetApproval($setUserApproval,$transDocSetApprove,$totalUser,$idUser,$date,$datetime);
        if($editOrInsertSetApproval==0){
            //echo "string";die();
            $model = $this->my_container_m_->insertSetApproval($setUserApproval,$transDocSetApprove,$totalUser,$idUser,$date,$datetime,$userAllowed);
        }else{
             //echo "rrr";die();
            $model = $this->my_container_m_->updateSetApproval($setUserApproval,$transDocSetApprove,$totalUser,$idUser,$date,$datetime,$userAllowed);
        }
        if($model == 1){
            $array = array(
            'act' => 1,
    ...
    public function updateApproval(){        
        ...
        $model = $this->my_container_m_->updateApproval($data,$id_approval);
        if ($model) {
            $array = array(
                'act'       => 1,
                'tipePesan' => 'success',
