...
class Update_deliver_nondoc extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('update_status/update_deliver_nondoc_m');  
    ...
    function home() {
        ...
        //echo $regNum;die();
        $getKetDoc    = $this->update_deliver_nondoc_m->getKetDoc($regNum);
        $getKetDocArr = explode('_', $getKetDoc);
        //print_r($getKetDocArr);die();
        $doc_description = $getKetDocArr[0];
        $username        = $getKetDocArr[1];

        $menuId = $this->home_m->get_menu_id('update_status/update_deliver_nondoc/home');
        ...
        if ($this->session->userdata('id_user') == '') {
            ...
            $data['regNum']          = $regNum;
            $this->template->set('title', 'home');
            $this->template->load('default_template/template9', 'update_status/update_deliver_nondoc_v', $data); 
        }
    ...
    function save() {
        ...
        $model = $this->update_deliver_nondoc_m->update_doc_reg($regNum,$data);
         if ($model) {
            $array = array(
                'act'        => 1,
                'tipePesan'  => 'success',
    ...
    function loginUpdateDeliver() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data ['regNum'] = $regNum;
            $this->load->view('login_template/login_update_nondoc_v', $data);
        } else {
