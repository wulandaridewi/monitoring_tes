...
class Delivery_internal_nondoc extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('update_status/delivery_internal_nondoc_m');  
    ...
    function home() {
        $menuId = $this->home_m->get_menu_id('update_status/delivery_internal_nondoc/home');
        ...
        $this->template->set('title', 'home');
        $this->template->load('default_template/template9', 'update_status/delivery_internal_nondoc_v', $data);
    }
    ...
    public function getDevAll() {
        ...
        $sCreateDate     = $post['columns'][7]['search']['value'];  
        $owner           = $post['columns'][3]['search']['value'];   
        $sDept           = $post['columns'][4]['search']['value'];     
        $sRecipt         = $post['columns'][6]['search']['value'];       
        // echo $dir;
        $rows = $this->delivery_internal_nondoc_m->getDevAll($dir,$start,$length,$sRegNum,$sCreateDate,$owner,$sDept,$sRecipt);
        $data = [];
        $no = 1;
        foreach ($rows as $row)
        {
            ...
            $data[] = [
                'no'                  => $no++,
                'register_non_doc_id' => trim($row->register_non_doc_id),
        ...
        $arrCompiledData = [           
            'recordsTotal'    => $this->delivery_internal_nondoc_m->getRecordsTotal(),
            'recordsFiltered' => $this->delivery_internal_nondoc_m->getRecordsFilteredTotal($dir,$start,$length,$sRegNum,$sCreateDate,$sExpedition,$sOwner,$sReceiptNum),
            'draw'            => $draw,
            'data'            => $data,
        ];
    ...
    public function uploadExcel(){
        try{
            ...
            $path       = "./assets/delivery_internal_nondoc/".date("Y-m-d");
            //$path   = realpath(FCPATH."/assets/delivery_internal_nondoc/".date("Y-m-d"));
            if(!is_dir($path)) {
            ...
            foreach($objPHPExcel->getWorksheetIterator() as $worksheet)
            {
                ...
                if($judul == "DATA DELIVERY INTERNAL NON DOC"){
                    for($row=4; $row<=$highestRow; $row++)
                    {
                        ...
                        $email    = $this->delivery_internal_nondoc_m->getEmail($Owner);
                        //KIRIM EMAIL
                        ...
                        $updateRow = $this->delivery_internal_nondoc_m->update_doc_reg($dataTable,$RegisterId);
                        if($updateRow == 'true'){
                            $countSuccess++;
                        }else{
                            $countError++;
