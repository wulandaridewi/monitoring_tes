...
class Pickup_non_doc extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('update_status/pickup_non_doc_m');  
    ...
    function home() {
        $menuId = $this->home_m->get_menu_id('update_status/pickup_non_doc/home');
        ...
        $this->template->set('title', 'home');
        $this->template->load('default_template/template9', 'update_status/pickup_non_doc_v', $data);
    }

    function getNoRegDoc() {
    	$regNum    		 = trim($this->input->post('regNum'));
    	$getKetDoc 		 = $this->pickup_non_doc_m->getKetDoc($regNum);
    ...
    function save() {
        ...
        $data = array(
            'npickup_by' => $pickup_by,            
            'nregister_status' => 1,
            'nstatus_email'    => $statusEmail,
        );
        $model = $this->pickup_non_doc_m->update_doc_reg($regNum,$data);
         if ($model) {            
