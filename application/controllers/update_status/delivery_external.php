...
class Delivery_external extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('update_status/delivery_external_m');  
    ...
    function home() {
        $menuId = $this->home_m->get_menu_id('update_status/delivery_external/home');
        ...
        $data['getExpedition'] = $this->delivery_external_m->getExpedition();
        $this->template->set('title', 'home');
        $this->template->load('default_template/template9', 'update_status/delivery_external_v', $data);
    }
    ...
    public function getDevAll() {
        ...
        // echo $dir;
        $rows = $this->delivery_external_m->getDevAll($dir,$start,$length,$sRegNum,$sCreateDate,$sExpedition,$sOwner,$sReceiptNum);
        $data = [];
        $no = 1;
        foreach ($rows as $row)
        {
        ...
        $arrCompiledData = [           
            'recordsTotal'    => $this->delivery_external_m->getRecordsTotal(),
            'recordsFiltered' => $this->delivery_external_m->getRecordsFilteredTotal($dir,$start,$length,$sRegNum,$sCreateDate,$sExpedition,$sOwner,$sReceiptNum),
            'draw'            => $draw,
            'data'            => $data,
        ];
    ...
    public function getDevAll1() {
        $rows = $this->delivery_external_m->getDevAll();
        $data['data'] = array();
        $no = 0;
        foreach ($rows as $row) {
            $no++;
    ...
    public function uploadExcel(){
        try{
            ...
            //echo $fileName;die();
            $path       = "./assets/delivery_external_doc/".date("Y-m-d");
            //$path   = realpath(FCPATH."/assets/delivery_external_doc/".date("Y-m-d"));
            if(!is_dir($path)) {
            ...
            foreach($objPHPExcel->getWorksheetIterator() as $worksheet)
            {
                ...
                if($judul == "DATA DELIVERY EXTERNAL"){
                    for($row=4; $row<=$highestRow; $row++)
                    {
                        ...
                        $DeliveryLocation    = $worksheet->getCellByColumnAndRow(3, $row)->getValue();
                        $Recipient           = $worksheet->getCellByColumnAndRow(4, $row)->getValue();  
                        $Owner               = $worksheet->getCellByColumnAndRow(5, $row)->getValue();
                        $Expedition          = $worksheet->getCellByColumnAndRow(6, $row)->getValue();
                        $estimasi            = $worksheet->getCellByColumnAndRow(7, $row)->getValue(); 
                        $RecipientNumber     = $worksheet->getCellByColumnAndRow(8, $row)->getValue(); 

                        $email    = $this->delivery_external_m->getEmail($Owner);
                        //KIRIM EMAIL
                        ...
                        //UPDATE KE DATABASE
                        $dataTable  = array(
                            'receipt_number'  => $RecipientNumber,
                            'register_status' => 1,
                            'status_email'    => $statusEmail,
                            'update_by'       => $idUser,
                            'update_date'     => $dateTime
                        );
                        $updateRow = $this->delivery_external_m->update_doc_reg($dataTable,$RegisterId);
                        if($updateRow == 'true'){
                            $countSuccess++;
                        }else{
                            $countError++;
